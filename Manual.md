...
| create-vertical-route     | name of route              |         |          | Creates a route by the given name from current position to a point above (or below) at the given distance relative to the constructs current orientation. |
|                           | -distance                  | number  | N        | The distance of the point above or below (when negative)                                                                                                  |
...
### Non gravity-aligned atmospheric accent/decent

While it is possible to make routes that are not gravity aligned work, they may be somewhat unreliable, especially when the construct is heavy. In short, this is mostly due to how atmospheric brakes function.
