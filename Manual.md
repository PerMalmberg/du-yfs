...
> Note: This only works if your route goes in a straight line, i.e. the route does not have a bend in the middle.
...
|                           | -pathAlignmentAngleLimit    | degrees       | Y        | The threshold angle that determines if the construct will align to the flight path or the gravity vector. Default: 40 degrees.                                                                       |
|                           | -pathAlignmentDistanceLimit | meter         | Y        | The threshold distance that determines if the construct will align to the flight path or the gravity vector. Default: 100m.                                                                          |
