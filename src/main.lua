...
function ActionStart(system, key)
    if key == "option1" then
        moveControl:Clear()
        moveControl:Append(MovementBehaviour(construct.position.Current(), startPos + upDirection * 2, PointAlongParallelLine, AboveSelfAlignedToGravity, 1, calc.Kph2Mps(5)))
    elseif key == "option2" then
        moveControl:Clear()
        moveControl:Append(MovementBehaviour(construct.position.Current(), startPos + upDirection * 10, PointAlongParallelLine, AboveSelfAlignedToGravity, 1, calc.Kph2Mps(20)))
    elseif key == "option3" then
        moveControl:Clear()
        moveControl:Append(MovementBehaviour(construct.position.Current(), startPos + upDirection * 300, PointAlongParallelLine, AboveSelfAlignedToGravity, 1, calc.Kph2Mps(50)))
    end
end
