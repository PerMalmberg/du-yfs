...
input:Register(keys.option8, Criteria():OnPress(), function()
    fc:ClearWP()
    fc:AddWaypoint(Waypoint(start - construct.world.GAlongGravity():normalize() * 1800, calc.Kph2Mps(1500), 0.1, alignment.RollTopsideAwayFromNearestBody, alignment.YawPitchKeepOrthogonalToGravity))
    fc:AddWaypoint(Waypoint(start - construct.world.GAlongGravity():normalize() * 2000, calc.Kph2Mps(1500), 0.1, alignment.RollTopsideAwayFromNearestBody, alignment.YawPitchKeepOrthogonalToGravity))
    fc:StartFlight()
end)

input:Register(keys.option9, Criteria():OnPress(), function()
    fc:ClearWP()
    fc:AddWaypoint(Waypoint(start - construct.world.GAlongGravity():normalize() * 200, calc.Kph2Mps(1500), 0.1, alignment.RollTopsideAwayFromNearestBody, alignment.YawPitchKeepOrthogonalToGravity))
